...
export default function GroupManagementModal({
  ...
  onClose,
  groups = [],
  users = [], // Now receiving users as props instead of fetching
  onCreateGroup,
  onDeleteGroup,
  onUpdateGroup,
}) {
  ...
  // Load users from localStorage when modal opens
  useEffect(() => {
    if (isOpen) {
      setLoading(true);
      try {
        // Try to get users from localStorage first
        const cached = localStorage.getItem('group_modal_users_cache');
        if (cached) {
          const parsedUsers = JSON.parse(cached);
          console.log("Loaded users from localStorage:", parsedUsers.length);
          setCachedUsers(Array.isArray(parsedUsers) ? parsedUsers : []);
        } else {
          console.log("No cached users found in localStorage, using props");
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {showForm
                ...
                : `View and manage all chat groups (${cachedUsers.length} users from cache)`}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              {loading ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    ...
                    <p className="mt-4 text-gray-600">Loading users from cache...</p>
                  </div>
                </div>
              ) : showForm ? (
                <GroupForm
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-500">
                  ...
                  <div className="mt-4 text-xs text-gray-400">
                    Available users from cache: {cachedUsers.length}
                  </div>
                  {cachedUsers.length === 0 && (
                    <div className="mt-2 text-xs text-amber-600">
                      No cached users found. Users need to be loaded in User Manager first.
                    </div>
                  )}
                </div>
              )}
